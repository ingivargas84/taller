...
<body>
    ...
    <table>
        ...
        @foreach($transacciones as $key => $mov) 
        @if($mov->tipo_transaccion_id == 1)
         <tr class="blue">
            <td>{{ $key + 1 }}</td>
            @if($mov->compra != null)
            <td>{{ $mov->compra->num_factura }}</td>
            @elseif($mov->abono->no_documento != null)
            <td>{{ $mov->abono->no_documento }}</td>
            @else
            <td>No aplica</td>
            @endif
             <td style="margin-right: 40px">{{ date("d/m/Y", strtotime($mov->fecha_transaccion)) }}</td>
             @if($mov->tipo_transaccion_id == 3)
                <td style="margin-right:80px">
                    {{$mov->tipoTransaccion->tipo_transaccion}}
                    ({{$mov->compra->num_factura}})
                </td>
             @elseif($mov->tipo_transaccion_id == 4)
                <td style="margin-right:80px">
                    @if($mov->abono->no_documento != null)
                    {{$mov->tipoTransaccion->tipo_transaccion}}
                    ({{$mov->abono->no_documento}})
                    @else
                    {{$mov->tipoTransaccion->tipo_transaccion}}
                    @endif
                </td>
            ...
            <td style="text-align: right;">{{ number_format($mov->total, 2, '.', ',')}}</td>
            <td style="text-align: center">Q.</td>
            <td style="text-align: right;">{{number_format($mov->saldo, 2,'.',',')}}</td>
            
        </tr>
    @elseif($mov->tipo_transaccion_id == 3 || $mov->tipo_transaccion_id == 4)
        <tr class="red">
            <td>{{ $key + 1 }}</td>
            @if($mov->compra != null)
            <td>{{ $mov->compra->num_factura }}</td>
            ...
            @endif
             ...
             @elseif($mov->tipo_transaccion_id == 4)
                <td style="margin-right:80px">
                 ...
                 @elseif($mov->tipo_transaccion_id == 2)
                <td style="margin-right:80px">
                    @if($mov->abono->documento_id === 1)
                    {{$mov->tipoTransaccion->tipo_transaccion}}
                    -Depósito
                    @elseif($mov->abono->documento_id === 2)
                    {{$mov->tipoTransaccion->tipo_transaccion}}
                    -Cheque
                    @else
                    {{$mov->tipoTransaccion->tipo_transaccion}}
