  ...
  <table>
    ...
    <table id="datos">
        ...
        @foreach($visitas as $v)
            <tr>
            <td style="width: 10%; height: 70px;  word-wrap: break-word;padding: 10px;">
                {{ date('d/m/Y', strtotime($v->fecha)) }}
            </td>
            <td style="width: 10%; height: 70px;  word-wrap: break-word;padding: 10px;">
            @if($v->ordenEquipo != null) 

                {{ strtoupper($v->ordenEquipo->no_orden_trabajo ) }}
            @else
                No establecido
            @endif
            </td>
            <td style="width: 15%; height: 70px;  word-wrap: break-word; padding: 10px;">
                {{ strtoupper($v->cliente->nombre_comercial) }}
            </td>
            <td style="width: 15%; height: 70px;  word-wrap: break-word;padding: 10px;">
                {{ strtoupper($v->cliente->direccion ) }}
            </td>
            <td style="height: 70px;  word-wrap: break-word; padding: 10px;">
                {{ strtoupper($v->cliente->telefono ) }}
            </td>
            <td style="width: 12%; height: 70px; padding: 10px; text-align:center; padding: 10px;  word-wrap: break-word;">
                {{ strtoupper($v->cliente->correo_electronico) }}
            </td>
            <td style="width: 15%; height: 70px; padding: 10px;  word-wrap: break-word;">
                {{ strtoupper($v->observaciones) }}
    ...
    <br>
    <br>
    <table style="margin-left: 2%; font-size: 12px; margin-top: 5% position: fixed; bottom: 0px">
        <tr>
        ...
        <tr>
            <td style="text-align: center;">{{ strtoupper(Auth::user()->name)}}</td>
        </tr>
    </table>
</body>
