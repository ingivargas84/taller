<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>Document</title>
    <style>
        table {
            border-collapse: collapse;
            margin-left: 20px;
            width: 100%;
            margin-right: 20px;
        }
    </style>
</head>

<body>
    <table style="margin-bottom: 40px">
        <tr>
            <td >
                <h1>Recibo 00{{ $cajaD[0]->numero}}-{{ $cajaD[0]->anio }}.</h1>
                <img style="float: right; margin-top: -80px; margin-bottom: 20px; height: 100px" src="data:image/jpeg;base64," height="160px" alt="">
            </td>
        </tr>
    </table>
    <table>
        <tr>
            <td style="padding: 10px;  border: 1px solid black">
                <span><b>Recibí de:</b> </span>
                <span style="float: right;">NAMSA ELECTRONICS</span><br> 
            </td>
            <td style="padding: 10px;  border: 1px solid black; text-align: right">
                Q. {{ number_format($cajaD[0]->total,2, '.', ',') }}
            </td>
        </tr>
    </table>
        <table>
            <tr>
                <td style="padding: 10px; border-right: 1px solid black; border-left: 1px solid black;  border-bottom: 1px solid black">
                    <span ><b>Fecha:</b>  </span>
                    <span style="float: right;">{{ date('d', strtotime($cajaD[0]->fecha)) }} de {{ $monthName }} de {{date('Y', strtotime($cajaD[0]->fecha )) }}</span><br>
                </td>
            </tr>
        </table>
        <table>
            <tr>
                <td
                    style="padding: 10px; border-right: 1px solid black; border-left: 1px solid black;  border-bottom: 1px solid black">
                    <span><b>Pagado a:</b> </span>
                    <span style="float: right;">{{ $cajaD[0]->receptor }}</span><br>
                </td>
            </tr>
        </table>
        <table>
            <tr>
                <td style="padding: 10px; padding-top: -30px; border-left: 1px solid black; border-bottom: 1px solid black;">
                    <span><b>Por concepto de:</b> </span>
                </td>
                <td
                    style="padding: 20px; border-right: 1px solid black; border-bottom: 1px solid black; height: 6%;">
                    <span style="float: right; ">{{ $cajaD[0]->descripcion }}</span><br>
                </td>
            </tr>
        </table>
    <table style="padding-left: 10px; margin-top: 70px;">
        <tr>
            <td
                style=" padding: 10px;">
                <span style="margin-left: 20%;"><b>Firma de recibido:</b> </span>
                <span>______________________________</span><br>
            </td>
        </tr>
    </table>
</body>
</html>